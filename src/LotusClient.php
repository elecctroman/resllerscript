...
final class LotusClient
{
        ...
        while (true) {
            ...
            try {
                ...
                if ($status >= 300 && $status < 400) {
                    ...
                    if ($location !== '') {
                        $next = $this->resolveRedirectTarget($currentPath, $location);
                        $loopKey = $this->loopKey($next);
                        if (isset($visited[$loopKey])) {
                            $this->logger->error('Lotus yönlendirme döngüsü algılandı: ' . $next);
                        } else {
                            $visited[$loopKey] = true;
                            $this->logger->info('Lotus yönlendirmesi algılandı, yeni hedef: ' . $next);
                            $currentPath = $next;
    ...
    {
        $uri = strpos($path, '://') !== false ? $path : $this->baseUri . ltrim($path, '/');
        if ($useQueryParam) {
    ...
    private function resolveRedirectTarget(string $current, string $location): string
    {
        ...
        if ($trimmed[0] === '/') {
            return $this->baseUri . ltrim($trimmed, '/');
        }

        // Göreceli yol
        $base = strpos($current, '://') !== false ? $current : $this->baseUri . ltrim($current, '/');
        ...
        $rebased = ($parsed['scheme'] ?? 'https') . '://' . ($parsed['host'] ?? '') . (isset($parsed['port']) ? ':' . $parsed['port'] : '') . $path . ltrim($trimmed, '/');
