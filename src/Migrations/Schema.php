...
final class Schema
{
    public static function ensure(): void
    {
        ...
        self::ensureResellersTable($pdo);
        self::ensureResellerApiKeysTable($pdo);
        self::ensureResellerApiLogsTable($pdo);
    ...
    private static function ensureResellersTable(PDO $pdo): void
    {
        ...
        self::ensureColumn($pdo, 'resellers', 'company', 'VARCHAR(150) DEFAULT NULL');
        self::ensureColumn($pdo, 'resellers', 'status', "ENUM('active','suspended','deleted') DEFAULT 'active'");
    }

    private static function ensureResellerApiKeysTable(PDO $pdo): void
    {
        $pdo->exec("CREATE TABLE IF NOT EXISTS api_keys (
            ...
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            CONSTRAINT fk_api_keys_reseller FOREIGN KEY (reseller_id) REFERENCES resellers(id) ON DELETE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

        self::ensureColumn($pdo, 'api_keys', 'allowed_ips', 'TEXT DEFAULT NULL');
        self::ensureColumn($pdo, 'api_keys', 'allowed_domains', 'TEXT DEFAULT NULL');
        self::ensureColumn($pdo, 'api_keys', 'status', "ENUM('active','revoked','suspended') DEFAULT 'active'");
        self::ensureColumn($pdo, 'api_keys', 'last_used_at', 'DATETIME DEFAULT NULL');
    }

    private static function ensureResellerApiLogsTable(PDO $pdo): void
    {
        $pdo->exec("CREATE TABLE IF NOT EXISTS api_logs (
            ...
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            INDEX idx_api_logs_key_created_at (api_key, created_at)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    }
