...
class Gateway
{
    ...
    public function getStatus(): array
    {
        ...
        if ($qr === null) {
            $qr = $this->captureScreenshotFallback($driver);
        }
        $this->markPending();
    ...
    private function captureQr(RemoteWebDriver $driver): ?string
    {
        try {
            ...
            if ($dataUrl !== null) {
                $this->logger->warning('Unexpected QR payload', ['payload_type' => gettype($dataUrl)]);
            }
        } catch (TimeoutException $exception) {
            $this->logger->warning('QR code not available yet', ['error' => $exception->getMessage()]);
        } catch (Throwable $exception) {
            $this->logger->error('QR capture failed', ['error' => $exception->getMessage()]);
        }

        return null;
    }

    private function captureScreenshotFallback(RemoteWebDriver $driver): ?string
    {
        try {
            $binary = $driver->takeScreenshot();
            if (is_string($binary) && $binary !== '') {
                return 'data:image/png;base64,' . base64_encode($binary);
            }
        } catch (Throwable $exception) {
            $this->logger->error('Screenshot fallback failed', ['error' => $exception->getMessage()]);
        }
