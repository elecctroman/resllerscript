...
class ProviderDispatchService
{
    ...
    public static function dispatchProductOrder($orderId)
    {
        ...
        $normalizedCode = strtolower($providerCode);
        if ($normalizedCode === '' || $normalizedCode === 'stock' || $normalizedCode === 'panel') {
        ...
        if ($normalizedCode !== self::PROVIDER_CODE) {
            return array('success' => false, 'reason' => 'Desteklenmeyen sağlayıcı.');
        }

        $providerStmt = $pdo->prepare('SELECT api_url, api_key, timeout_ms FROM providers WHERE name = :name AND status = 1 LIMIT 1');
        $providerStmt->execute(array(':name' => 'Lotus'));
        $provider = $providerStmt->fetch(PDO::FETCH_ASSOC);

        if (!$provider) {
            return array('success' => false, 'reason' => 'Lotus sağlayıcısı yapılandırılmamış.');
        }

        $apiUrl = isset($provider['api_url']) ? trim((string) $provider['api_url']) : '';
        ...
        $connectTimeout = min($timeoutMs, 10000);

        if ($apiUrl === '' || $apiKey === '') {
            return array('success' => false, 'reason' => 'Sağlayıcı API bilgileri eksik.');
        }

        $logger = new Logger(dirname(__DIR__, 2) . '/storage/lotus.log');

        try {
            if (class_exists('GuzzleHttp\\Client')) {
                $client = new LotusClient($apiUrl, $apiKey, $timeoutMs, $connectTimeout, $logger);
            } else {
                $client = new LotusClientCurl($apiUrl, $apiKey, $timeoutMs, $connectTimeout, $logger);
            }
        ...
        if (!isset($response['success']) || !$response['success']) {
            $message = isset($response['message']) ? (string) $response['message'] : 'Sağlayıcı siparişi oluşturulamadı.';
            self::storeProviderFailure($orderId, $order, $message);
