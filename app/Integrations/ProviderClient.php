...
class ProviderClient
{
    ...
    private function request(string $method, string $path, array $payload = array()): array
    {
        ...
        do {
            if (isset($visited[$url])) {
                if ($this->looksLikeLoginUrl($url)) {
                    $this->log('error', 'Sağlayıcı API giriş sayfasına yönlendiriyor.', array('url' => $url, 'base_url' => $this->baseUrl));
                ...
                throw new RuntimeException('Sağlayıcı API yönlendirme döngüsü algılandı. Lütfen temel URL ve kimlik bilgisi ayarlarınızı kontrol edin.');
            }
    ...
    private function looksLikeLoginUrl(string $url): bool
    {
        ...
        return str_contains($lower, '/login')
            || str_contains($lower, 'giris')
            || str_contains($lower, 'signin')
