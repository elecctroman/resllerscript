...
Sistemi SMM sağlayıcı olarak kullanmak isteyen paneller **mutlaka** `/api/v2` uç noktasına istek göndermelidir. (Geriye dönük uyumluluk için `/api/smm.php` halen çalışsa da Perfect Panel ve benzeri paneller varsayılan olarak `/api/v2` dizinini arar.) Tüm isteklerde `key` parametresiyle bayi profil ekranından alınan API anahtarı gönderilmelidir. Yanıt formatı Perfect Panel uyumlu olacak şekilde düzenlenmiştir; hata durumlarında `{"error": "..."}` döner, servis listesi `service`, `name`, `category`, `type`, `rate`, `min`, `max`, `desc`, `currency` alanlarını içerir ve sipariş durumları `Pending`, `Processing`, `Completed`, `Canceled`, `Partial`, `Refunded` gibi Perfect Panel’in beklediği statü etiketleriyle döner. Desteklenen işlemler:

> Doğru uç noktaya ulaştığınızı doğrulamak için tarayıcıdan `https://alanadiniz.com/api/v2` adresini açtığınızda `{"error":"API key is missing"}` cevabını görmelisiniz. Bu yanıt gelmiyorsa URL yanlış veya sunucu yapılandırması uygun değildir.
