    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!Helpers::verifyCsrf($csrfToken)) {
            ...
            if ($action === 'save_provider') {
                ...
                $baseUrl = normalizeProviderBaseUrl((string) ($_POST['base_url'] ?? ''));
            ...
            <div class="card-body">
                <form method="post" class="row g-3">
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" id="provider-name" name="name" class="form-control" value="<?= Helpers::sanitize($selectedProvider['name'] ?? '') ?>" required>
                    </div>
                    <div class="col-12 col-md-6">
                    ...
                    <div class="col-12">
                        ...
                        <input type="url" id="provider-base-url" name="base_url" class="form-control" placeholder="https://partner.lotuslisans.com.tr" value="<?= Helpers::sanitize($selectedProvider['base_url'] ?? '') ?>" required>
                    </div>
                    <div class="col-12">
                        <label for="provider-api-key" class="form-label">API Anahtarı</label>
                        <input type="text" id="provider-api-key" name="api_key" class="form-control" value="<?= Helpers::sanitize($selectedProvider['api_key'] ?? '') ?>" required>
                    </div>
                            ...
                            <table class="table table-sm align-middle">
                                ...
                                <tbody>
                                    <?php foreach ($productResult['items'] as $product) : ?>
                                        <tr>
                                            <td><?= Helpers::sanitize($product['id'] ?? '-') ?></td>
                                            <td>
                                                <strong><?= Helpers::sanitize($product['title'] ?? '-') ?></strong>
                                                <?php if (!empty($product['content'])) : ?>
                                                    <div class="small text-muted"><?= Helpers::sanitize(providerExcerpt((string) $product['content'])) ?></div>
                                                <?php endif; ?>
                                            </td>
                                            <td><?= Helpers::sanitize($product['amount'] ?? '-') ?></td>
                                            <td><?= Helpers::sanitize($product['stock'] ?? '-') ?></td>
 ...
 */
function getProvider(PDO $pdo, int $id)
{
...
function insertProvider(PDO $pdo, string $name, string $baseUrl, string $apiKey, int $isActive)
{
...
function updateProvider(PDO $pdo, int $id, string $name, string $baseUrl, string $apiKey, int $isActive)
{
...
function deleteProvider(PDO $pdo, int $id)
{
    $stmt = $pdo->prepare('DELETE FROM external_providers WHERE id = :id');
    $stmt->execute(array('id' => $id));
}

function normalizeProviderBaseUrl(string $value)
{
    $value = trim($value);
    if ($value === '') {
        return '';
...
function providerExcerpt(string $value, int $limit = 60)
{
    $value = trim($value);
...
function formatJsonOutput($decoded, string $raw)
{
...
function performApiTest(string $baseUrl, string $apiKey)
{
    ...
    if ($response['error']) {
        ...
        if (isset($decoded['success']) && $decoded['success']) {
            $message = 'API bağlantısı başarılı. Bakiye: ' . ($decoded['data']['credit'] ?? 'bilinmiyor');
...
function fetchProviderProducts(string $baseUrl, string $apiKey)
{
...
function lotusApiRequest(string $baseUrl, string $apiKey, string $method, string $path, ?array $payload = null)
{
