    ...
    if ($action === 'create') {
        ...
        if (!$errors) {
            ...
            if (!$errors && $userId !== null) {
                ...
                if ($userRecord) {
                    $notificationSent = ResellerNotifier::sendOnboarding(
                        $userRecord,
                        array('package_name' => 'Panel erişimi', 'price' => 0),
                        $password
                    );
                }

                $success = $notificationSent
                    ? 'Bayi hesabı oluşturuldu ve Telegram bildirimi gönderildi.'
                    : 'Bayi hesabı oluşturuldu. Telegram bildirimi yapılandırılmadıysa lütfen kullanıcıyı manuel bilgilendirin.';
