...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_category') {
        ...
        if (!$errors) {
            ...
            $pdo->prepare('INSERT INTO products (name, category_id, cost_price_try, price, description, sku, status, created_at) VALUES (:name, :category_id, :cost_price_try, :price, :description, :sku, :status, NOW())')->execute([
                'name' => $name,
        ...
        if (!$errors) {
            ...
            $pdo->prepare('UPDATE products SET name = :name, category_id = :category_id, cost_price_try = :cost_price_try, price = :price, description = :description, sku = :sku, status = :status, updated_at = NOW() WHERE id = :id')->execute([
                'id' => $productId,
                'name' => $name,
                'category_id' => $categoryId,
                'cost_price_try' => $costPriceTry,
            ...
            <div class="card-body">
                <form method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="cost_price_try" class="form-control" value="0" required>
                        <small class="text-muted">Satış fiyatı güncel kur ve %<?= Helpers::sanitize(number_format(Helpers::commissionRate(), 2, ',', '.')) ?> komisyon oranı eklenerek otomatik hesaplanır.</small>
                    </div>
                    <div class="mb-3">
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <?php foreach ($products as $product): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if (isset($product['cost_price_try']) && $product['cost_price_try'] !== null): ?>
                                        <?= Helpers::sanitize(number_format((float)$product['cost_price_try'], 2, ',', '.')) ?> ₺
                                    <?php else: ?>
                                        ...
                                        <form method="post">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-4">
                                                        <label class="form-label">Alış Fiyatı (₺)</label>
                                                        <input type="number" step="0.01" min="0" name="cost_price_try" class="form-control" value="<?= Helpers::sanitize(isset($product['cost_price_try']) ? $product['cost_price_try'] : '') ?>" required>
                                                        <small class="text-muted d-block">Mevcut satış fiyatı: <?= Helpers::sanitize(number_format((float)$product['price'], 2, '.', ',')) ?> $</small>
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label class="form-label">SKU</label>
