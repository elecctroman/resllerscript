...
if ($isAjax) {
    ...
    if ($action === 'fetch_translations') {
        try {
            ...
            $availableLanguages = array_map(static function ($language) {
                return isset($language['code']) ? strtolower((string) $language['code']) : '';
            }, LanguageService::languages(true));
            ...
            error_log('[languages] Çeviri kataloğu yüklenemedi: ' . $exception->getMessage());
            echo json_encode(array(
                'success' => false,
                'message' => 'Çeviri verileri alınırken bir hata oluştu. Lütfen daha sonra tekrar deneyin.',
            ));
        }
        exit;
    }
    ...
    if ($action === 'save_translation') {
        ...
        try {
            ...
            error_log('[languages] Çeviri kaydedilemedi: ' . $exception->getMessage());
            echo json_encode(array('success' => false, 'message' => 'Çeviri kaydedilirken bir hata oluştu.'));
            exit;
        }
    ...
    if ($action === 'delete_translation') {
        ...
        try {
            ...
            error_log('[languages] Çeviri silinemedi: ' . $exception->getMessage());
            echo json_encode(array('success' => false, 'message' => 'Çeviri silinirken bir hata oluştu.'));
            exit;
        }

        echo json_encode(array('success' => $deleted, 'csrf_token' => Helpers::csrfToken()));
        exit;
    }
        ...
        function loadTranslations(locale) {
            ...
            fetch(window.location.href, {
                ...
                if (data.csrf_token) {
                    ...
                    tableContainer.setAttribute('data-csrf', csrf);
                }
