...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($orderId <= 0) {
        ...
        try {
            ...
            $orderStmt = $pdo->prepare('SELECT po.*, u.name AS user_name, u.email AS user_email, u.id AS owner_id, u.notify_order_completed, p.name AS product_name, p.sku, c.name AS category_name FROM product_orders po INNER JOIN users u ON po.user_id = u.id INNER JOIN products p ON po.product_id = p.id LEFT JOIN categories c ON p.category_id = c.id WHERE po.id = :id FOR UPDATE');
            ...
            if (!$order) {
                ...
                if ($currentStatus === $newStatus) {
                    ...
                    if (!$userRow) {
                        $pdo->rollBack();
                        $formErrors[] = 'Bayi bilgilerine ulaşılamadı.';
                    } else {
                        ...
                        if ($currentStatus === 'cancelled' && $newStatus !== 'cancelled') {
                            if ($userBalance < $amount) {
                                $pdo->rollBack();
                                $formErrors[] = 'Bayi bakiyesi yeniden tahsilat için yetersiz.';
                            } else {
                        ...
                        if (!$formErrors) {
                            ...
                            if ($newStatus === 'completed') {
                                ...
                                ResellerMailer::sendOrderCompleted($orderPayload, $userPayload);
