...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$request) {
        ...
        try {
            ...
            if ($status === 'approved' && !$isPackageOrder && $displayEmail !== '') {
                $userPayload = array(
                    'email' => $displayEmail,
                    'name' => isset($request['user_name']) ? $request['user_name'] : null,
                    'notify_balance_approved' => isset($request['notify_balance_approved']) ? $request['notify_balance_approved'] : null,
                    'telegram_bot_token' => isset($request['telegram_bot_token']) ? $request['telegram_bot_token'] : null,
                    'telegram_chat_id' => isset($request['telegram_chat_id']) ? $request['telegram_chat_id'] : null,
                );
                ...
                ResellerNotifier::sendBalanceApproved($userPayload, $requestPayload, $adminNote);
            }
