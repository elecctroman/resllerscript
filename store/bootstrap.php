...
if (!function_exists('store_header_categories')) {
    /**
     * @return array<int,array<string,string|int>>
     */
    function store_header_categories(): array
    {
        static $cache = null;
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                $normalized[] = array(
                    ...
                    'name' => $name,
                    'slug' => $slug,
                    'url' => url_category(array('id' => $id, 'slug' => $slug, 'name' => $name)),
                );
            }

            $cache = $normalized;
        } catch (\PDOException $exception) {
            error_log('[Storefront] Kategori menüsü yüklenemedi: ' . $exception->getMessage());
            $cache = array();
        }
