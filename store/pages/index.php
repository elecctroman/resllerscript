...
if ($customCollections) {
    ...
    foreach ($customCollections as $collection) {
        ...
        $featuredCollections[] = array(
            ...
            'url' => $url !== '' ? $url : store_url('category.php'),
...
if ($customSlides) {
    foreach ($customSlides as $slide) {
        ...
        $url = isset($slide['url']) ? (string) $slide['url'] : store_url('category.php');
        if ($url !== '' && !preg_match('/^https?:/i', $url)) {
            $url = store_url(ltrim($url, '/'));
        }
...
if (!$heroSlides) {
    ...
    foreach (array_slice($fallbackSource, 0, 3) as $item) {
        ...
        $heroSlides[] = array(
            ...
            'cta' => 'İncele',
            'url' => isset($item['url']) ? (string) $item['url'] : store_url('category.php'),
            'tag' => 'Öne Çıkan',
