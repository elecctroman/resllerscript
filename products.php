                ...
                if (!$product) {
                    ...
                    if (!$freshUser) {
                        ...
                        if ($price > $currentBalance) {
                            ...
                            $orderStmt = $pdo->prepare('INSERT INTO product_orders (product_id, user_id, note, price, status, created_at) VALUES (:product_id, :user_id, :note, :price, :status, NOW())');
                            $orderStmt->execute([
...
try {
    ...
    if ($selectedCategoryId) {
        ...
        $productsQuery .= ' AND pr.category_id IN (' . $placeholders . ')';
        $productParams = array_merge($productParams, $categoryIds);
    }
    ...
    <div class="col-12 col-lg-3">
        <div class="card border-0 shadow-sm">
            ...
            <div class="list-group list-group-flush">
                <a href="/products.php" class="list-group-item list-group-item-action <?= $selectedCategoryId === 0 ? 'active' : '' ?>">
                    Tüm Ürünler
                </a>
                <?php foreach ($flattenedCategories as $category): ?>
                    <a href="/products.php?category=<?= (int)$category['id'] ?>"
                       ...
                       style="padding-left: <?php echo 18 + ($category['depth'] * 16); ?>px;">
            ...
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <?php if ($selectedCategoryId): ?>
                        <small class="text-muted">Filtre: <?= Helpers::sanitize($categoryPath($selectedCategoryId)) ?></small>
                    <?php endif; ?>
                </div>
                        ...
                        <table class="table table-hover align-middle">
                            ...
                            <?php foreach ($products as $product): ?>
                                <tr>
                                    ...
                                    <td><?= Helpers::sanitize(isset($product['sku']) ? $product['sku'] : '-') ?></td>
