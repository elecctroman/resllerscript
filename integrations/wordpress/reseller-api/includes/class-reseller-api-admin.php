...
class Reseller_API_Admin
{
    ...
    public function sanitize_settings($settings)
    {
        ...
        $settings['api_url'] = isset($settings['api_url']) ? esc_url_raw($settings['api_url']) : '';
    ...
    public function render_field_url()
    {
        ...
        printf('<input type="url" class="regular-text" name="%1$s[api_url]" value="%2$s" placeholder="https://panel.example.com/" />', esc_attr(Reseller_API_WooCommerce_Plugin::OPTION_KEY), esc_attr($settings['api_url']));
    }
    ...
    public function ajax_fetch_balance()
    {
        ...
        $balance = isset($profile['data']['credit']) ? $profile['data']['credit'] : 0;
        $email = isset($profile['data']['email']) ? $profile['data']['email'] : '';
