    ...
    <div class="app-main d-flex flex-column flex-grow-1">
        <?php if ($user): ?>
            <header class="app-topbar">
                ...
                <form method="post" action="/language.php" class="d-flex align-items-center ms-auto">
                    <input type="hidden" name="csrf_token" value="<?= Helpers::csrfToken() ?>">
                    <input type="hidden" name="redirect" value="<?= Helpers::sanitize(Helpers::currentPath()) ?>">
                    <select name="locale" class="form-select form-select-sm" onchange="this.form.submit();">
                        <?php $activeLocale = Lang::locale(); ?>
                        <option value="tr" <?= $activeLocale === 'tr' ? 'selected' : '' ?>><?= Helpers::sanitize('Türkçe') ?></option>
                        <option value="en" <?= $activeLocale === 'en' ? 'selected' : '' ?>><?= Helpers::sanitize('English') ?></option>
                    </select>
                </form>
            </header>
        <?php endif; ?>
