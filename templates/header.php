...
<div class="app-shell d-flex flex-column min-vh-100">
            ...
            <nav class="navbar navbar-expand-xl navbar-dark app-navbar">
                <div class="container-xxl">
                    <div class="d-flex align-items-center gap-3">
                        <a class="navbar-brand fw-semibold" href="<?= $isAdminArea ? '/admin/dashboard.php' : '/dashboard.php' ?>">
                            <?= Helpers::sanitize($siteName) ?>
                        </a>
                        <?php if ($siteTagline): ?>
                            <span class="app-navbar-tagline text-white-50 d-none d-xxl-inline"><?= Helpers::sanitize($siteTagline) ?></span>
                        <?php endif; ?>
                    </div>
                    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#appPrimaryNav" aria-controls="appPrimaryNav" aria-expanded="false" aria-label="<?= Helpers::sanitize('Menüyü Aç') ?>">
                        <span class="navbar-toggler-icon"></span>
                    </button>
                    <div class="collapse navbar-collapse" id="appPrimaryNav">
                        <ul class="navbar-nav app-menu me-xl-auto mb-3 mb-xl-0">
                            <?php foreach ($menuSections as $section): ?>
                                <?php
                                $sectionActive = false;
                                foreach ($section['items'] as $navItem) {
                                    if (Helpers::isActive($navItem['pattern'])) {
                                        $sectionActive = true;
                                        break;
                                    }
                                }
                                ?>
                                <li class="nav-item dropdown app-menu-section">
                                    <a class="nav-link dropdown-toggle <?= $sectionActive ? 'active' : '' ?>" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <?= Helpers::sanitize($section['heading']) ?>
                                    </a>
                                    <div class="dropdown-menu app-dropdown shadow-lg">
                                        <?php foreach ($section['items'] as $navItem): ?>
                                            <?php $itemActive = Helpers::isActive($navItem['pattern']); ?>
                                            <a class="dropdown-item d-flex align-items-center gap-2 <?= $itemActive ? 'active' : '' ?>" href="<?= $navItem['href'] ?>">
                                                <?php if (!empty($navItem['icon'])): ?>
                                                    <i class="<?= Helpers::sanitize($navItem['icon']) ?> text-primary"></i>
                                                <?php endif; ?>
                                                <span><?= Helpers::sanitize($navItem['label']) ?></span>
                                                <?php if (!empty($navItem['badge']) && (int) $navItem['badge'] > 0): ?>
                                                    <span class="badge bg-primary-subtle text-primary ms-auto"><?= (int) $navItem['badge'] ?></span>
                                                <?php endif; ?>
                                            </a>
                                        <?php endforeach; ?>
                                    </div>
                        ...
                        <div class="app-navbar-actions d-flex flex-column flex-xl-row align-items-start align-items-xl-center gap-3 ms-xl-4">
                            <?php if (Helpers::featureEnabled('balance')): ?>
                                <div class="app-balance-pill d-flex align-items-center gap-2">
                                    <i class="bi bi-wallet2"></i>
                                    <span><?= Helpers::sanitize('Bakiye') ?>:</span>
                                    <strong><?= Helpers::formatCurrencyHtml((float) $user['balance']) ?></strong>
                                </div>
                            <?php endif; ?>
                            <?php if ($showLanguageSwitch): ?>
                                <div class="app-language-switcher w-100 w-xl-auto">
                                    <label class="form-label form-label-sm text-white-50 mb-1" for="appLanguageSelect"><?= Helpers::sanitize('Dil') ?></label>
                            ...
                            <?php endif; ?>
                            <div class="dropdown w-100 w-xl-auto">
                                <button class="btn btn-light btn-sm d-flex align-items-center gap-2 w-100 app-user-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                    <span class="avatar-circle fw-semibold"><?= Helpers::sanitize($avatarInitial) ?></span>
                                    <span class="d-none d-sm-inline text-start">
                                        <span class="d-block text-dark fw-semibold"><?= Helpers::sanitize($user['name']) ?></span>
                                        <small class="text-muted"><?= Helpers::sanitize($userRoleLabel) ?></small>
                                    </span>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end shadow app-user-menu">
                                    <li class="dropdown-header">
                                        <div class="fw-semibold"><?= Helpers::sanitize($user['name']) ?></div>
                                        <small class="text-muted"><?= Helpers::sanitize($userRoleLabel) ?></small>
                                    </li>
                                    <li><hr class="dropdown-divider"></li>
                                    <li><a class="dropdown-item" href="/profile.php"><i class="bi bi-person me-2"></i><?= Helpers::sanitize('Profilim') ?></a></li>
                                    <?php if ($isAdminRole && !$isAdminArea): ?>
                                        <li><a class="dropdown-item" href="/admin/dashboard.php"><i class="bi bi-speedometer2 me-2"></i><?= Helpers::sanitize('Yönetim Paneli') ?></a></li>
                                    <?php endif; ?>
                                    <li><a class="dropdown-item" href="/logout.php"><i class="bi bi-box-arrow-right me-2"></i><?= Helpers::sanitize('Çıkış Yap') ?></a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <?php else: ?>
        <header class="app-header app-header--guest shadow-sm">
            <nav class="navbar navbar-light app-navbar">
                <div class="container-xxl d-flex align-items-center justify-content-between">
                    <a class="navbar-brand fw-semibold" href="/"><?= Helpers::sanitize($siteName) ?></a>
                    <?php if ($siteTagline): ?>
                        <span class="app-navbar-tagline text-muted small d-none d-sm-inline"><?= Helpers::sanitize($siteTagline) ?></span>
                    <?php endif; ?>
                </div>
            </nav>
        </header>
    ...
    <main class="app-main flex-grow-1">
        <div class="app-content container-xxl py-4">
        <?php if ($user): ?>
            <div class="app-page-header d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-4">
                <div>
                    <h1 class="h3 mb-1"><?= Helpers::sanitize($pageHeadline) ?></h1>
                    <p class="text-muted mb-0 small"><?= date('d F Y') ?></p>
                </div>
                <?php if (Helpers::featureEnabled('balance')): ?>
                    <span class="badge text-bg-primary rounded-pill px-4 py-2 d-lg-none">
                        <?= Helpers::sanitize('Bakiye') ?>: <span class="fw-semibold"><?= Helpers::formatCurrencyHtml((float) $user['balance']) ?></span>
                    </span>
                <?php endif; ?>
            </div>
        <?php endif; ?>
