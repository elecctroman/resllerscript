...
if (!function_exists('smm_success')) {
    function smm_success($payload)
    {
        json_response($payload, 200);
    }
}

if (!function_exists('smm_error')) {
    function smm_error($message, array $context = array())
    {
        $response = array_merge(array('error' => $message), $context);
        json_response($response, 200);
    }
}

if (!function_exists('smm_normalize_action')) {
    function smm_normalize_action($action)
    {
        $action = strtolower((string)$action);

        $map = array(
            'services' => array('services', 'service', 'getservices', 'service_list', 'services_list', 'list', 'products'),
            'add' => array('add', 'order', 'create', 'place_order', 'addorder'),
            'status' => array('status', 'order_status', 'getstatus', 'check', 'checkstatus', 'orderstatus'),
            'multi_status' => array('multi_status', 'multistatus', 'status_multi', 'status_mult', 'orders', 'orders_status', 'statusorders', 'multi-status'),
            'balance' => array('balance', 'getbalance', 'balance_status', 'check_balance'),
            'refill' => array('refill', 'refill_status', 'refillstatus', 'multirefill', 'refill_multi', 'multirefillstatus', 'refill_status_multi', 'refillstatusmulti'),
            'cancel' => array('cancel', 'cancel_order', 'cancelorders'),
        );
...
if ($key === '') {
    smm_error('API key is missing');
}

$token = App\ApiToken::findActiveToken($key);
if (!$token) {
    smm_error('Invalid API key.');
}

if ($action === '') {
    smm_error('Action parameter is required.');
}

$pdo = App\Database::connection();
$normalizedAction = smm_normalize_action($action);
...
switch ($normalizedAction) {
    case 'services':
        try {
            ...
            $productStmt = $pdo->prepare('SELECT pr.*, cat.name AS category_name FROM products pr LEFT JOIN categories cat ON pr.category_id = cat.id WHERE pr.status = :status ORDER BY cat.name ASC, pr.name ASC');
            ...
            smm_success($services);
        } catch (\PDOException $exception) {
            smm_error('Unable to load services: ' . $exception->getMessage());
        }
        break;
    ...
    case 'add':
        ...
        if ($serviceId <= 0) {
            smm_error('Service parameter is invalid.');
        }
        ...
        try {
            ...
            $productStmt = $pdo->prepare('SELECT pr.*, cat.name AS category_name FROM products pr LEFT JOIN categories cat ON pr.category_id = cat.id WHERE pr.id = :id AND pr.status = :status LIMIT 1 FOR UPDATE');
            $productStmt->execute(array('id' => $serviceId, 'status' => 'active'));
            $product = $productStmt->fetch();

            if (!$product) {
                $pdo->rollBack();
                smm_error('Service not found.');
            }
            ...
            if ($quantity < $minQuantity || $quantity > $maxQuantity) {
                $pdo->rollBack();
                smm_error(sprintf('You can order between %d and %d units for this service.', $minQuantity, $maxQuantity));
            }
            ...
            if (!$userRow) {
                $pdo->rollBack();
                smm_error('Reseller account could not be found.');
            }
            ...
            if ($totalCost > $currentBalance) {
                $pdo->rollBack();
                smm_error('Insufficient balance.');
            }
            ...
            smm_error('Order could not be created: ' . $exception->getMessage());
        }
        break;
    ...
    case 'status':
        ...
        if ($orderId <= 0) {
            smm_error('Order parameter is invalid.');
        }

        try {
            $stmt = $pdo->prepare('SELECT po.*, pr.name AS product_name FROM product_orders po LEFT JOIN products pr ON pr.id = po.product_id WHERE po.id = :id AND po.user_id = :user_id LIMIT 1');
            $stmt->execute(array('id' => $orderId, 'user_id' => $token['user_id']));
            $order = $stmt->fetch();

            if (!$order) {
                smm_error('Order not found.');
            }
            ...
            smm_error('Unable to load order status: ' . $exception->getMessage());
        }
        break;
    ...
    case 'multi_status':
        ...
        if (empty($ids)) {
            smm_error('Orders parameter is required.');
        }
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT po.*, pr.name AS product_name FROM product_orders po LEFT JOIN products pr ON pr.id = po.product_id WHERE po.id IN (' . $placeholders . ') AND po.user_id = ? ORDER BY po.id ASC');
            $stmt->execute($params);
            ...
            smm_error('Unable to load orders: ' . $exception->getMessage());
        }
        break;
    ...
    case 'balance':
        try {
            ...
            $balanceValue = $balance !== false ? (float)$balance : 0.0;

            smm_success(array(
                'balance' => number_format($balanceValue, 4, '.', ''),
                'balance_float' => round($balanceValue, 4),
                'currency' => smm_currency(),
            ));
        } catch (\PDOException $exception) {
            smm_error('Unable to fetch balance: ' . $exception->getMessage());
        }
        break;
    ...
    case 'cancel':
        smm_error('This feature is not available for your account.');
        break;

    default:
        smm_error('Unsupported action: ' . $action);
}
