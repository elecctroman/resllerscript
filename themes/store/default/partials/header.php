...
<header class="store-header" data-store-header>
    <div class="store-topbar">
        <div class="container-xxl d-flex align-items-center gap-3">
            ...
            <div class="store-actions-mobile d-lg-none ms-auto d-flex align-items-center gap-2">
                ...
                <button class="btn btn-outline-secondary" type="button" data-bs-toggle="offcanvas" data-bs-target="#storeMobileMenu" aria-controls="storeMobileMenu" aria-label="Menüyü aç">
            ...
            <div class="container-xxl">
                <ul class="store-nav__list" role="menubar">
                    <?php foreach ($megaMenuGroups as $index => $group): ?>
                        ...
                        ?>
                        <li class="store-nav__item" role="none">
                            <button class="store-nav__button" type="button" data-nav-button aria-expanded="false" aria-controls="<?= htmlspecialchars($controlId, ENT_QUOTES, 'UTF-8') ?>">
                                <span><?= htmlspecialchars($groupName, ENT_QUOTES, 'UTF-8') ?></span>
                                <i class="bi bi-chevron-down" aria-hidden="true"></i>
                            </button>
                            <div class="store-nav__dropdown" id="<?= htmlspecialchars($controlId, ENT_QUOTES, 'UTF-8') ?>" role="menu" hidden>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="storeMobileMenu" aria-labelledby="storeMobileMenuLabel">
        <div class="offcanvas-header">
            <h2 class="offcanvas-title h5" id="storeMobileMenuLabel">Menü</h2>
        ...
        <div class="offcanvas-body d-flex flex-column gap-4">
            ...
            <?php if ($hasMegaMenu): ?>
                <div class="accordion" id="mobileMegaMenu">
                    <?php foreach ($megaMenuGroups as $index => $group): ?>
                        <?php
                        $groupId   = (int) $group['id'];
                        $groupName = isset($group['name']) ? (string) $group['name'] : 'Kategori';
                        $items     = isset($group['items']) && is_array($group['items']) ? $group['items'] : array();
                        $headingId = 'mobileMegaHeading-' . $groupId . '-' . $index;
                        $collapseId = 'mobileMegaCollapse-' . $groupId . '-' . $index;
                        ?>
                        <div class="accordion-item">
                            <h3 class="accordion-header" id="<?= htmlspecialchars($headingId, ENT_QUOTES, 'UTF-8') ?>">
                            ...
                            <div id="<?= htmlspecialchars($collapseId, ENT_QUOTES, 'UTF-8') ?>" class="accordion-collapse collapse" aria-labelledby="<?= htmlspecialchars($headingId, ENT_QUOTES, 'UTF-8') ?>" data-bs-parent="#mobileMegaMenu">
