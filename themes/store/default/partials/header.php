...
$homeUrl = store_url('');
$searchAction = store_url('category');
$loginUrl = isset($viewData['loginUrl']) ? (string) $viewData['loginUrl'] : store_url('account/login');
$registerUrl = isset($viewData['registerUrl']) ? (string) $viewData['registerUrl'] : store_url('account/register');
$accountUrl = isset($viewData['accountUrl']) ? (string) $viewData['accountUrl'] : store_url('account');
$ordersUrl = isset($viewData['ordersUrl']) ? (string) $viewData['ordersUrl'] : store_url('account/orders');
...
if (!$headerCategories) {
    ...
    foreach ($defaultCategories as $categoryName) {
        $headerCategories[] = array(
            'name' => $categoryName,
            'url' => store_url('category?q=' . rawurlencode($categoryName)),
            'icon' => '',
        );
    }
}
                ...
                <?php if ($isLoggedIn): ?>
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end user-menu shadow-sm border-0 rounded-3">
                            ...
                            <li><hr class="dropdown-divider"></li>
                            <li>
                                <form method="post" action="<?php echo htmlspecialchars($logoutAction, ENT_QUOTES, 'UTF-8'); ?>">
                ...
                <div class="py-4 d-lg-none">
                    ...
                    <?php if ($isLoggedIn): ?>
                        <div class="list-group mb-3">
                            ...
                            <form method="post" action="<?php echo htmlspecialchars($logoutAction, ENT_QUOTES, 'UTF-8'); ?>">
                                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($logoutToken, ENT_QUOTES, 'UTF-8'); ?>">
                                <button type="submit" class="list-group-item list-group-item-action text-danger">Çıkış Yap</button>
                            </form>
                        </div>
                    <?php else: ?>
