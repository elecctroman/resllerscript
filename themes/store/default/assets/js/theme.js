(function () {
    ...
    (function initPrimaryNav() {
        ...
        var dropdownWrappers = Array.prototype.slice.call(document.querySelectorAll('[data-dj-dropdown]'));
        var mobileTrigger = document.querySelector('[data-dj-mobile-trigger]');
        var mobilePanel = document.querySelector('[data-dj-mobile]');
        var mobileClose = document.querySelector('[data-dj-mobile-close]');
        var mobileBackdrop = document.querySelector('[data-dj-mobile-backdrop]');
        var openDropdown = null;
        var repositionScheduled = false;

        function hideDropdown(wrapper) {
            if (!wrapper) { return; }
            var trigger = wrapper.querySelector('[data-dj-trigger]');
            var menu = wrapper.querySelector('.dj-dd__menu');
            if (trigger) {
        ...
        function positionDropdown(menu, trigger) {
            ...
            if (top + menuRect.height > viewportHeight - offset) {
                var flipped = triggerRect.top - menuRect.height - offset;
                if (flipped >= offset) {
                    top = flipped;
                } else {
                    top = clamp(viewportHeight - menuRect.height - offset, offset, viewportHeight - menuRect.height - offset);
                }
            }

            var left = triggerRect.left;
            left = clamp(left, offset, viewportWidth - menuRect.width - offset);
        ...
        function scheduleReposition() {
            if (!openDropdown || repositionScheduled) { return; }
            repositionScheduled = true;
            window.requestAnimationFrame(function () {
                repositionScheduled = false;
                if (!openDropdown) { return; }
                var trigger = openDropdown.querySelector('[data-dj-trigger]');
                var menu = openDropdown.querySelector('.dj-dd__menu');
                if (menu && trigger) {
                    positionDropdown(menu, trigger);
                }
            });
        ...
        document.addEventListener('click', function (event) {
            if (!openDropdown) { return; }
            var wrapper = event.target.closest('[data-dj-dropdown]');
            if (!wrapper || wrapper !== openDropdown) {
                closeAllDropdowns();
            }
        });
        ...
        function openMobileMenu() {
            ...
            html.classList.add('dj-mobile-open');
        }
        ...
        function closeMobileMenu() {
            ...
            if (mobileBackdrop) {
                ...
                mobileBackdrop.setAttribute('hidden', 'hidden');
            }
            html.classList.remove('dj-mobile-open');
        }

        if (mobileTrigger && mobilePanel) {
            mobileTrigger.addEventListener('click', function (event) {
                event.preventDefault();
                var expanded = mobileTrigger.getAttribute('aria-expanded') === 'true';
                if (expanded) {
                    closeMobileMenu();
                } else {
                    openMobileMenu();
