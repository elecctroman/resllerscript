(function () {
    ...
    (function initMegaNav() {
        ...
        function positionPortalDropdown(dropdown, trigger) {
            ...
            var margin = 8;
            var offsetAttr = dropdown.getAttribute('data-offset') || trigger.getAttribute('data-offset') || '8';
            var offset = parseInt(offsetAttr, 10);
            ...
            window.requestAnimationFrame(function () {
                ...
                var left;
                if (alignPref === 'center') {
                    left = triggerBox.left + (triggerBox.width / 2) - (dropdownWidth / 2);
                } else if (alignPref === 'end' || (alignPref === '' && direction === 'rtl')) {
                    left = triggerBox.right - dropdownWidth;
                } else {
                    left = triggerBox.left;
                }

                var leftMin = margin;
                var leftMax = viewportWidth - margin - dropdownWidth;
                if (leftMax < leftMin) { leftMax = leftMin; }
                if (left < leftMin) { left = leftMin; }
                if (left > leftMax) { left = leftMax; }

                var top = triggerBox.bottom + offset;
                var bottomLimit = viewportHeight - margin;
                if (top + dropdownHeight > bottomLimit) {
                    var flippedTop = triggerBox.top - dropdownHeight - offset;
                    if (flippedTop >= margin) {
                        top = flippedTop;
                    } else {
                        top = Math.max(margin, bottomLimit - dropdownHeight);
                    }
                }
                if (top < margin) {
                    top = margin;
