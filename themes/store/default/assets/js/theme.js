(function () {
    ...
    (function initMegaNav() {
        ...
        function resetDropdownAlignment(dropdown) {
            if (!dropdown) { return; }
            dropdown.classList.remove('is-align-right', 'is-align-left');
            dropdown.style.removeProperty('left');
            dropdown.style.removeProperty('right');
        }

        function updateDropdownAlignment(dropdown) {
            if (!dropdown || !DESKTOP_QUERY.matches) { return; }
            resetDropdownAlignment(dropdown);
            window.requestAnimationFrame(function () {
                var rect = dropdown.getBoundingClientRect();
                var viewportWidth = window.innerWidth || document.documentElement.clientWidth || 0;
                if (!rect || !viewportWidth) { return; }
                var margin = 16;
        ...
        window.addEventListener('resize', function () {
            navItems.forEach(function (item) {
                if (item.classList.contains('is-open')) {
                    updateDropdownAlignment(item.querySelector('.store-nav__dropdown'));
                }
            });
        });
